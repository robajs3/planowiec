...
class ActivityService:
    ...
    def ensure_default_types() -> None:
        ...
        if ActivityType.query.filter_by(is_default=True).first():
            return
        for name, color, icon in DEFAULT_ACTIVITY_TYPES:
            db.session.add(ActivityType(name=name, color=color, icon=icon, owner_id=None, is_default=True))
        db.session.commit()
    ...
    def delete_type(user_id: int, type_id: int) -> tuple[bool, str | None]:
        activity_type = ActivityType.query.get(type_id)
        if not activity_type or activity_type.is_default or activity_type.owner_id != user_id:
